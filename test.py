...
search_query = '''index="jenkins" event_tag="build_summary" sourcetype="json:jenkins" 
...
splunk_search_base_url = f'https://bgl-vms-vm1221:8089/servicesNS/{username}/search/search/jobs'.format(username)
...
while is_job_completed != 'DONE':
    ...
    job_status_base_url = f'https://bgl-vms-vm1221:8089/servicesNS/{username}/search/search/jobs/{unique_id}'.format(username, unique_id)
    ...
    print(f"Current Job Status is {is_job_completed}")

splunk_summary_base_url = f'https://bgl-vms-vm1221:8089/servicesNS/{username}/search/search.jobs/{unique_id}/results'.format(username, unique_id)
splunk_summary_results = requests.get(splunk_summary_base_url, params=get_data, verify=True, auth=(username, password))
